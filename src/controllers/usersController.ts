...
export class UserController {
  ...
  async getUserByUID(req: Request, res: Response): Promise<Omit<IUser,'firebaseUID'> | undefined | null> {
    const userId = req.params.id;
    try {
      const user = await User.findOne({firebaseUID:userId},{email:1,firstName:1,lastName:1,nickName:1,avatarUrl:1,_id:0});
      res.json(user);
      return user;
    } catch (err) {
